...
def process_patient_state(patient, visit_date, visit)
  program_id = Connection.select_one("SELECT patient_program_id FROM patient_program
             WHERE patient_id = #{patient} AND program_id = 1 AND voided = 0
                 ORDER BY patient_program_id DESC LIMIT 1")

  if program_id.blank?
    puts "i am blank"
  else
    patient_state = patient_state = Connection.select_one("SELECT current_state_for_program(#{patient}, 1, '#{visit_date}') AS state")
    if patient_state.blank?
      puts "am a blank state"
    else
      if patient_state['state'] == "Unknown"
        state_name = "Unknown"
      else
        state = Connection.select_one("SELECT c.name AS name FROM program_workflow_state pws
                                        INNER JOIN concept_name c ON c.concept_id = pws.concept_id
                                          AND c.voided = 0 AND pws.retired = 0
                                  WHERE program_workflow_state_id = #{patient_state['state']}
                                      and program_workflow_id = 1 LIMIT 1")
        state_name = state['name']
      end #end patient state Unknown
    end #end patient_state
  end #end program_id blank
  ...
  unless patient_record.blank?
      Connection.execute <<EOF
UPDATE flat_table2
SET  current_hiv_program_state = "#{state['name']}", current_hiv_program_start_date = '#{visit_date}'
...
VALUES ("#{patient}", '#{visit_date}', "#{state['name']}", '#{visit_date}');
EOF
...
EOF
        ...
        #flat_table2_record = Connection.select_one("SELECT patient_id, visit_date, guardian_present, guardian_present_enc_id
        #                                FROM flat_table2
        #              WHERE  ID = #{flat_table_2_data['ID']}")
...
EOF
        ...
        #flat_table2_record = Connection.select_one("SELECT patient_id, visit_date, patient_present, patient_present_enc_id
        #                                FROM flat_table2
        #              WHERE  ID = #{visit}")
...
def process_hiv_clinic_registration_obs(encounter, visit)
  ...
  (patient_obs || []).each do |patient|
    #puts "concept_id #{patient['concept_id']}"
    patient_visit_date = patient['obs_datetime'].to_date.strftime("%Y-%m-%d")
...
=begin
    ...
    when cd4_count_less_than_or_equal_to_350
      if patient['voided'].to_i == 0
        value_numeric = patient['value_numeric']
        value_modifier = patient['value_modifier']
        puts "........... Updating record into flat_table1 (cd4_count = #{value_numeric}): #{patient['person_id']}"
...
EOF
    ...
    when cd4_count_less_than_or_equal_to_500
      if patient['voided'].to_i == 0
        value_numeric = patient['value_numeric']
        value_modifier = patient['value_modifier']
        puts "........... Updating record into flat_table1 (cd4_count = #{value_numeric}): #{patient['person_id']}"
...
EOF
    ...
    when cd4_count_less_than_or_equal_to_250
      if patient['voided'].to_i == 0
        value_numeric = patient['value_numeric']
        value_modifier = patient['value_modifier']
        puts "........... Updating record into flat_table1 (cd4_count = #{value_numeric}): #{patient['person_id']}"
...
EOF
    ...
    when who_stages_criteria_present
      if patient['voided'].to_i == 0
        ...
        Connection.execute <<EOF
UPDATE flat_table1
SET who_stages_criteria_present = "#{answer}", reason_for_starting_v_date = DATE('#{patient_visit_date}'), who_stages_criteria_present_enc_id = "#{patient['encounter_id']}"
WHERE flat_table1.patient_id = #{patient['person_id']};
EOF
      else
        ...
        Connection.execute <<EOF
UPDATE flat_table1
SET  who_stages_criteria_present = NULL, reason_for_starting_v_date = NULL, who_stages_criteria_present_enc_id = NULL
WHERE flat_table1.patient_id = #{patient['person_id']} ;
EOF
      end #voided
...
EOF
        ...
        if patient['voided'] == "0"
          ...
          Connection.execute <<EOF
UPDATE flat_table2
SET  type_of_ARV_regimen_given = '#{answer_record['name']}', type_of_ARV_regimen_given_enc_id = '#{patient['encounter_id']}'
WHERE flat_table2.id = '#{patient_check['ID']}';
EOF
        else #else voided
          Connection.execute <<EOF
...
def process_art_adherence_obs(encounter, visit)
  ...
  (patient_obs || []).each do |patient|
    ...
    when amount_of_drug_brought_to_clinic
      ...
      flat_table2_record = Connection.select_one("SELECT * FROM flat_table2 WHERE ID = #{visit['ID']} ")
...
def updating_patient_program_table(patient_ids)
  ...
  (data || []).each do |row|
      ...
      earliest_start_date_2 = row[:earliest_start_date].to_date
      date_enrolled_2       = row[:date_enrolled].to_date
...
def get_earliest_start_date_patients
  ...
  where
    (`p`.`voided` = 0)
        ...
        OR DATE(`p`.`date_changed`) = DATE('#{Current_date}'))
        OR ( DATE(`encounter`.`date_created`) = DATE('#{Current_date}')
        OR DATE(`encounter`.`date_changed`) = DATE('#{Current_date}')
...
def get_earliest_start_date_patients_data(patient_ids)
  records = Connection.select_all("select
    `p`.`patient_id` AS `patient_id`,
    `pe`.`gender` AS `gender`,
    `pe`.`birthdate`,
    date_antiretrovirals_started(`p`.`patient_id`, min(`s`.`start_date`)) AS `earliest_start_date`,
