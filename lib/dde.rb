module DDE
  ...
  def self.search_and_or_create(json)
    ...
    if result.blank?
      ...
      defidtype = PatientIdentifierType.find_by_name("Unknown ID") rescue nil

      (person["patient"]["identifiers"] rescue []).each do |identifier|
      
        if !local["patient"]["identifiers"].include?(identifier)
         
          idtype = PatientIdentifierType.find_by_name(identifier.keys[0]) rescue nil
          ...
          if (idtype.present? || defidtype.present?) && identifier[identifier.keys[0]].present?
            ...
            PatientIdentifier.create(
                ...
                "identifier_type" => (defidtype.id || idtype.id),
                "uuid" => uuid
            )
  ...
  def self.set_identifier(identifier, value, patient_id)
    patient_identifier_type = PatientIdentifierType.find_by_name(identifier)

    if value.present? && patient_identifier.present? && patient_identifier_type.present?

      PatientIdentifier.create(:patient_id => patient_id, 
                               :identifier => value,
                               :identifier_type => patient_identifier_type.id)
